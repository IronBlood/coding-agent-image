...
- [x] Install the Debian runtime utilities required by version 1: `bash`, `git`, `sed`, `awk`, `ripgrep`, `fd`, `build-essential`, and `libpcre2-dev`.
...
- [x] Install Debian `build-essential` so Rust projects have a linker and common native build tools.
- [x] Install Debian `libpcre2-dev` for workflows that need PCRE2 headers and libraries.
